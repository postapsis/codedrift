...
Do the following, in order:
  ...
  6. Submit them in that order: for each changeset call add_changeset({ reviewId: "${reviewId}", name, description, files }). Each entry in files must have:
    ...
    - \`comments\` (optional): line-anchored review comments where you have something worth pointing out (potential bugs, risky patterns, notable decisions). Each comment has a 1-based lineNumber, the comment text (markdown), and a side: use "new" (the default) with new-file line numbers for added/changed lines, and "old" with old-file line numbers for deleted lines. Only comment on lines that are part of the diff. See the "Rules for reviewing the changesets" section for reviewing rules.
