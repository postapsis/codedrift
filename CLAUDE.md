...
5. The `/dashboard/repositories` and `/dashboard/reviews` route files own their CRUD (TanStack Query + `useMutation`, raw `fetch` via `src/service/{repository,review}-service.ts`, base URL `VITE_API_BASE_URL`, default `http://localhost:3000`) and the add/delete shadcn `Dialog`s, delegating list rendering to presentational components (`src/components/repository-list.tsx`, `src/components/review/review-list.tsx`). Creating a review names it and adds one-or-more repositories — each row (`src/components/review/review-repository-entry.tsx`) loads that repo's branches (`GET /repository/:id/branches`, keyed `["branches", repositoryId]`) and picks base/head; the dialog's per-row form state uses the `ReviewEntry` type (`src/@types/review-entry.ts`). Success toasts fire via **Sonner** — `<Toaster/>` is mounted once in `__root.tsx`. Frontend shared types mirror the backend in `src/@types/` (`repository.ts`, `review.ts`, `changeset.ts`, `api-response.ts`). The `/dashboard/reviews/$reviewId` route fetches a review's changesets (`src/service/changeset-service.ts`, keyed `["changesets", reviewId]`) and, when empty, renders `src/components/review/review-setup.tsx` — the AI-agent instruction block with a copy-to-clipboard button.
