...
6. `src/tools/changeset-tools.ts` wraps git operations as `ai`-SDK tools (list commits, hunks, file content at base/head). These are exposed over HTTP by `src/routes/changeset-routes.ts` (`changesetRoutes`, registered in `main.ts`) under `POST /tools/changeset/*` (all-commits, hunk, file-content-at-base/head, add, files).
